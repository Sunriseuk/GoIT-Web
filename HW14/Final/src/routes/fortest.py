...
async def get_users_by_some_info(some_info: str, db: Session, user: AuthUser) -> List[User]:
    ...
    """
    response = []
    info_by_first_name = db.query(User).filter(
        and_(User.first_name.like(f'%{some_info}%'), User.authuser_id == user.id)).all()
    if info_by_first_name:
        for n in info_by_first_name:
            response.append(n)
    info_by_second_name = db.query(User).filter(
        and_(User.second_name.like(f'%{some_info}%'), User.authuser_id == user.id)).all()
...
async def get_birthday_per_week(days: int, db: Session, user: AuthUser) -> User:
    ...
    for usr in all_users:
        if timedelta(0) <= (
                (usr.birthaday.replace(year=int((datetime.now()).year))) - datetime.now().date()) <= timedelta(days):
            response.append(usr)
